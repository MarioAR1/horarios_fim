  ...
  <div class="content-wrapper">
    ...
    <section class="content container-fluid">
      ...
      if (isset($_POST['datos']) || isset($ses->selected))
      {
        ...
        if (!is_int($plan) || !is_int($esp) || $plan > 3 || $plan < 1 || $esp > 6 || $esp < 3 || $cruces < 1 || $cruces > 3)
        {
        ...
        $select_cursos = '<div class="row"><div class="col-md-12">
        ...
        for ($i = 1; $i <= 10; $i++)
        {
          ...
          {
            $select_cursos .= "<label class=\"col-md-2 control-label\" for=\"cursos\"></label><label><input type=\"checkbox\" name=\"cursos[]\" value=\"".$row["codigo"]."\"".(in_array($row["codigo"], $ses->selected) ? "checked=1" : "")."/> ".$row["codigo"].' - '.$row["nombre"]." (".$row["creds"]." crédito".($row["creds"] + 0 > 1 ? "s" : "").")</label><br>";
          }
        }
        
        $select_cursos .= '<h1><small>Electivos</small></h1>';
        $stmt->query("SELECT codigo,nombre,creds FROM cursosfim WHERE ".$malla[0].strval($i).$malla[1].($malla[1] == "" ? "" : strval($i))."' GROUP BY codigo,nombre,creds ORDER BY codigo ASC");
        //$stmt->query("SELECT codigo,nombre FROM cursosfim WHERE ".$malla."='11' GROUP BY codigo ORDER BY codigo ASC");
        while($row = $stmt->fetch())
        {
          $select_cursos .= "<label class=\"col-md-2 control-label\" for=\"cursos\"></label><label><input type=\"checkbox\" name=\"cursos[]\" value=\"".$row["codigo"]."\"".(in_array($row["codigo"], $ses->selected) ? "checked=1" : "")."/> ".$row["codigo"].' - '.$row["nombre"]." (".$row["creds"]." crédito".($row["creds"] + 0 > 1 ? "s" : "").")</label><br>";
        }
              ...
              <form class="form-horizontal" method="post">
                ...
                <div class="form-group">
                  ...
                  <div class="col-md-4">
                    ...
                    <div class="radio"><b>
                      Nota: No se generarán horarios que incumplan el reglamento de matrícula.
                    </b></div>
                  </div>
                </div>
